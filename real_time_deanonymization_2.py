...
class Deanonymize:
    ...
    def deanonymize(self,frame):
        ...
        if not is_random_mac:
            ...
            if frame['mac_address'] == '52:54:00:e3:d7:db':
                print('')
            ...
            if matching_IE_clusters:
                ...
                for clusters in self._clusters.values():
                    match ,_ = self.compare(frame,cluster)
                    if match:
    ...
    def compare_sequence_number(self, frame,cluster, max_seq_diff=50, max_time_diff=500, return_distance=False):
        ...
        distance = seq_number_check = abs(frame['sequence_number'] - cluster._max_seq_number)
    ...
    def compare_relative_distance(self, frame, clusters):
        ...
        for x in cluster_differences:
            match = self.compare_sequence_number(frame, x[0])
            if match:
                return x[0]
